    ...
    try{       
                                ...
                                if(rs.next()){
                                        if(request.getParameter("code")!=null  &&  request.getParameter("ptr")!=null ){
                                                          ...
                                                          ResultSet rs1 = st1.executeQuery("select * from cuserlike where email='"+email+"' AND code='"+code+"'");
                                                          if(rs1.next()){ 
                                                              pattern =rs1.getString("ptr");
                                                          }
                                                          if(ptr.equals(pattern)){
                                                                                PreparedStatement ps =cn.prepareStatement("delete from cuserlike  where email='"+email+"' AND code='"+code+"'"); 
                                                                                if(ps.executeUpdate()>0){
                                                                                     out.print("black"); 
                                                                                } 
                                                          }    
                                                          else if(pattern.equals("like")){ 
                                                                                PreparedStatement ps =cn.prepareStatement("update cuserlike set ptr=?   where email=? AND code=?"); 
                                                                    ...
                                                                    if(ps.executeUpdate()>0){
                                                                         out.print("blue");
                                                                            ...
                                                                            String like=""; 
                                                                               ...
                                                                               ResultSet rs3 = st3.executeQuery("select  count(*) from cuserlike where code='"+code+"' AND ptr='like'");
                                                                               if(rs3.next()){
